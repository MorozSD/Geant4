...
void FitData(G4double *x,G4double *y,G4double *coefbeta,const size_t k, G4double *erry,G4int Array_Size,G4int Track_ID,G4double& R2Adj,G4double& RSS){
            ...
            // G4cout << "Polynomial fit Y(x)!" << G4endl;
             ...
             //     G4double x[5] = {1,2,3,4,5};
              //    G4double y[5] = {1,4,9,16,25};
   // Input values
 // **************************************************************
                            
    // Definition of other variables
    // **************************************************************
    		 size_t n = Array_Size;                                    // Number of data points (adjusted later)
    		 size_t nstar = 0;                                // equal to n (fixed intercept) or (n-1) not fixed    
    
//G4cout << " coef = " << coefbeta[0] << " " << coefbeta[1] << " " << coefbeta[2] << k << " " << erry[0] << G4endl; 
                    
                 //G4double coefbeta[k+1];
    ...
    // **************************************************************
   		 ...
   		 RSS = CalculateRSS(x,y,coefbeta,Weights,std::fixed,n,k+1);
   		 ...
   		 R2Adj = CalculateR2Adj(x,y,coefbeta,Weights,fixedinter,n,k+1);
   		 
    //G4cout << " R2Adj " << R2Adj << " RSS " << RSS << G4endl;             
    if ((nstar-k)>0) {
...
G4double okr(G4double value, G4int numer) {
  				return (value / std::fabs(value)) * round(std::fabs(value) * std::pow(10, numer)) / std::pow(10, numer);
 }
