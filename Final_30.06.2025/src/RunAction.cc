...
RunAction::RunAction()
{
 ...
 // analysisManager->CreateH1("NofHits","Hits in one volume", 9, -0.5, 8.5);
 ...
 // analysisManager->CreateH1("Start Time","Energy distribution", 100, -1.*MeV, 5.*MeV);
  analysisManager->CreateH1("Length","Shortest distance", 100, -0.5*mm, 5*mm);
 // analysisManager->CreateH1("Length_0","Shortest distance", 100, -0.5*mm, 5*mm);
 // analysisManager->CreateH1("Length_10","Shortest distance", 100, -0.5*mm, 5*mm);
 // analysisManager->CreateH1("Length_100","Shortest distance", 100, -0.5*mm, 5*mm);
  //analysisManager->CreateH1("Length_1000","Shortest distance", 100, -0.5*mm, 5*mm);
 // analysisManager->CreateH1("Length_2000","Shortest distance", 100, -0.5*mm, 5*mm);
 // analysisManager->CreateH1("Edep","Edeposit distribution", 100, -100.*eV, 60*keV);
 
 //  analysisManager->CreateH1("Length","Parent's ID", 100, -1., 1000.);
 /* analysisManager->CreateH1("Time3","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time4","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time5","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time6","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time7","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time8","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time9","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time10","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time11","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time12","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time13","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time14","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time15","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time16","Timing distribution with drift", 120, -1.*ns, 1200.*ns);*/
  // analysisManager->CreateH1("theta pz","Z-Z0, theta", 364, 0.5*radian, 3.14159/2*radian+0.1);
  //analysisManager->CreateH1("theta z,y","Z-Z0, theta", 364, 0.5*radian, 3.14159*radian+0.1);
  /*analysisManager->CreateH1("Time7","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time8","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time9","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time10","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  ...
  analysisManager->CreateH1("Time14","Timing distribution with drift", 120, -1.*ns, 1200.*ns);
  analysisManager->CreateH1("Time15","Timing distribution with drift", 120, -1.*ns, 1200.*ns);*/
  //analysisManager->CreateH1("Edep","Edeposit distribution", 100, -10.*eV, 60*keV);
   //analysisManager->CreateH1("Znach","Z_nach", 240, -60*cm, 60*cm);
 //  analysisManager->CreateH1("Z1","Z", 240, -5*cm, 60.*cm);
  // analysisManager->CreateH1("Z_result","Z0", 240, -60*cm, 60.*cm);
  // analysisManager->CreateH1("Z_0-Z_01","Z01", 100, -0., 8*mm);
   
  // analysisManager->CreateH1("Z_nach","Znach", 100, -80.*cm, 80*cm);
   analysisManager->CreateH2("Z0-theta dist pZ","Z-Z0, theta", 364, 0.4*radian, 3.14159*radian-0.4*radian, 120, -1*cm, 1*cm); //Dphi2
  // analysisManager->CreateH2("Z0-theta dist ZY","Z-Z0, theta", 364, 0.4*radian, 3.14159*radian-0.4*radian, 120, -1*cm, 1*cm); //Dphi1
   
   //analysisManager->CreateH2("T-Z0","T, Z", 120, -60*cm, 60*cm, 120, -10.*ns, 1200.*ns);
 //  analysisManager->CreateH2("Z0-time","Z0, time", 500, 0, 1000, 120, -30*cm, 30*cm);
 //  analysisManager->CreateH2("Zraz-R2Adj","Z-Z0, R2Adj", 364, 0.88, 1.0, 120, -20*cm, 20*cm);
