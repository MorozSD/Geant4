...
//  G4cout << "i after if = " << i << G4endl;
   ...
   for (k = i+1; k<nofHits; k++)
    ...
    //     G4cout << " STEP = " << step << G4endl;
     ...
     for (size_t Poly = 2; Poly<3; Poly++){     
          size_t k = Poly;                            // Polynomial order
          G4cout << "POLY = " << Poly << G4endl;
    if(ID_Number > k){      
   ...
   // yX[ii] = ii*ii + 0.5;
    ...
    // Display statistics
    // **************************************************************
 //   DisplayStatistics(n,nstar,k,RSS_X,R2_X,R2Adj_X,SE_X);
...
//  G4int J = Poly-1; 
    ...
    // Display ANOVA table
    // **************************************************************
  //  DisplayANOVA(nstar, k, TSS_X, RSS_X, J);
 ...
 //   DisplayANOVA(nstar, k, TSS, RSS);
  ...
  //  WriteCIBands("CIBands2.dat",x,coefbeta,XTWXInv,tstudentval,SE,n,k);
   ...
   // DisplayCovCorrMatrix(k, SE, fixedinter, XTWXInv);
	...
	G4cout << " T in the end = " << G4BestUnit(Zx,"Length")<< G4endl;
	
	//analysisManager->FillH1(0, Zx);
  
      //  analysisManager->FillNtupleDColumn(0, Zx);
	//analysisManager->AddNtupleRow();
	//G4cout << " Z0 by X(z) = " << Zx << G4endl;
     //   analysisManager->FillH1(0, Zx);

	G4double Zy = SetStartingPoint(k, coefbeta_Y, ID_Number, xY);
	G4double detZx, detZy;
	G4int justTrackID1 = (*fHitsCollection)[step]->GetTrackID();
	G4int justTrackID0 = (*fHitsCollection)[step-1]->GetTrackID();
	G4cout << " PREV_ID = " << justTrackID0 << " NEXT_ID = " << justTrackID1 << G4endl;
	G4ThreeVector VertexZ0 = (*fHitsCollection)[step-1]->GetTrackVertex();
	
	if (VertexZ0.x() == 0 && VertexZ0.y() == 0){
	G4cout << "Z0 = " << G4BestUnit(VertexZ0.z(),"Length") << " X0 = " << VertexZ0.x() << " Y0 = " << VertexZ0.y() << G4endl;
	detZx = Zx-VertexZ0.z();
	G4cout << " detZ in the end = " << G4BestUnit(detZx,"Length")<< G4endl;
	detZy = Zy-VertexZ0.z();
	analysisManager->FillH1(0, detZx);
	analysisManager->FillH1(1, detZy);
	}
}	
//	analysisManager->FillH1(2, Zy);
  
 //       analysisManager->FillNtupleDColumn(2, Zy);
 //       analysisManager->AddNtupleRow();
