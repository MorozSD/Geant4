...
G4double SetLength(const size_t k,const G4double *coefbeta, G4double x){ //for x, y
 ...
 /* const G4int m = k; 
  ...
  for (G4int i = 0; i<m+1; i++){
    KOEF[i] = coefbeta[i];
    G4cout << " coef = " << KOEF[i] << G4endl;
   }
   G4cout <<" KOEF[1]" << KOEF[1] << " KOEF[2]" << KOEF[2] << G4endl;
 */// G4double Z = x[0];
  G4double C = coefbeta[1]+2*coefbeta[2]*x;
 // G4cout << " C = " << C << G4endl; 
  G4double C1 = std::sqrt(1+C*C);
 // G4cout << " C1 = " << C1 << G4endl;
 // G4double Co = coefbeta[2]*(coefbeta[1]*std::sqrt(1+coefbeta[1]*coefbeta[1])+std::log(std::fabs(coefbeta[1]+std::sqrt(1+coefbeta[1]*coefbeta[1]))))/4; 
 // G4double Aabs = std::abs(KOEF[0]);
 // G4cout << " const = " << Co << G4endl;
 // G4double Tan = (-2*Aabs*x+C1)/KOEF[1];
 // G4cout << " Tan = " << Tan << G4endl;
 // t = (1/8)*(2*C*C1/KOEF[0]+(2/KOEF[0])*(Tan+std::pow(Tan,3)/3+std::pow(Tan,5)/5)-(1/Aabs)*std::log(-KOEF[1]-2*Aabs*x+C1)+std::log(KOEF[1]-2*Aabs*x+C1));
  t = (std::log(std::fabs(C1+C))+C*C1)/(4*coefbeta[2]);
 // G4cout << " t = " << t << G4endl; 
  //G4cout <<" KOEF[1]" << coefbeta[1]<< " KOEF[2]" << coefbeta[2] << G4endl;
