    ...
    // if L>0  }
      ...
      //формула для 1 точки - Пифагор, ибо z один => делаем прямая лежит в плоскость z=z2, тогда расстояние находим между точками x2,y2,z2, x,y,z, где z2=z,  а знаяит, уходит
       ...
       /*      for(G4int i = 0; i < i_step ; i++ ){
		...
		if (first) {
		first = false;
		stuff_XY << "#,x,y,Track_ID" << std::endl;
		stuff_Zl << "#,l,z,Track_ID" << std::endl;
		}
         ...
         //       G4cout  << " ID = " << prevTrackID << " "<< G4endl;
            ...
            //        	(*fHitsCollection)[k]->GetChamberNb();
                      ...
                      //   if(point_check.x()*point_check.y()>0){
                         	x[ii] = pointH.x();
                         	y[ii] = pointH.y(); // function x(t)
                         	//chamb = (*fHitsCollection)[Array[ii-1]]->GetChamberNb();
                         	//G4cout << " Chamb = " << chamb << " x= " << x[ii]<< " ID = " << prevTrackID<< G4endl;
                       //  	stuff_XY << "," << x[ii] << "," << y[ii] << "," << prevTrackID<< std::endl; 
                   //      	G4cout << " x " << x[ii] << " y " << y[ii] << G4endl;
                     //    }
                    /*     else{
                         //	G4cout << " Inverse !! " << " ID = " << prevTrackID<< G4endl;
                         	x[ii] = pointH.y();
                ...
                //    G4cout << "Polynomial fit Y(x)!" << G4endl;
                    ...
                    G4double R2Adj_x = 0.,R2Adj_y = 0., R2Adj_z = 0.; 
                    FitData(x,y,coefbeta,k_fit,erry,Array_Size,prevTrackID,R2Adj_x);  
         ...
         //	    	G4cout << " WARNING !!!" << G4endl;
         	    	FitData(x,y,coefbeta_inv,k_fit,erry,Array_Size,prevTrackID,R2Adj_y);  
         ...
         //       G4cout << "Polynomial fit Z(l)!" << G4endl;
                ...
                FitData(l,z,coefbeta_Z,k_Z,erry_Z,Array_Size-1,prevTrackID,R2Adj_z); 
       ...
       //         G4cout << " Track_ID " << prevTrackID << " Zraz = " << G4BestUnit(Zraz,"Length")<< G4endl; 
                if(Dphi2>=0.5){
              
             //   G4cout << " R2_Z = " << R2Adj_z << G4endl;
             //   All_Track_dphi++;
              //  analysisManager->FillH2(2, R2Adj_x, Zraz);
                if(std::fabs(Zraz) > 1*cm){ 
                G4cout << " AAAAAAAA Track_ID " << prevTrackID << " Zraz = " << G4BestUnit(Zraz,"Length") << " Z_nach " << Z_nach << G4endl; 
                 	for (G4int ii = 1; ii<Array_Size; ii++){
                
                		stuff_XY << "," << x[ii] << "," << y[ii] << "," << prevTrackID<< std::endl; 
                		stuff_Zl << "," << l[ii-1] << "," << z[ii-1] << "," << prevTrackID << std::endl;
                	}  
        ...
        /*         G4cout << " R2_y(x) = " << R2Adj_x << " R2_x(y) = " << R2Adj_y << " Zraz = " << Zraz << " ID " << prevTrackID<< G4endl;	
                 G4cout << "KOEF " << coefbeta[0] << "," << coefbeta[1] << "," << coefbeta[2]<< G4endl;*/
                }
