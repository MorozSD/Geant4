...
G4double SetStartingTime  (const size_t k, const G4double *coefbeta){ //for x, y
  G4double t1, t2;
  t1 = -coefbeta[1] + std::sqrt(pow(coefbeta[1],2)-4.0*coefbeta[0]*coefbeta[2]);
  t2 = -coefbeta[1] - std::sqrt(pow(coefbeta[1],2)-4.0*coefbeta[0]*coefbeta[2]);
...
G4double SetStartingPoint  (const size_t k, const G4double *coefbeta){ return coefbeta[0]; }	
